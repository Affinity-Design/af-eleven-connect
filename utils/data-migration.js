      ...
      if (client.metricsHistory && client.metricsHistory.length > 0) {
        ...
        for (const entry of client.metricsHistory) {
          ...
          if (!entry.agentId || !entry.period || !entry.metrics) {
            console.log(
              `[Data-Migration] Removing invalid entry without basic fields`
            );
            removedCount++;
            continue;
          }
          ...
          if (!metrics.agentId || !metrics.year || !metrics.month) {
            console.log(
              `[Data-Migration] Fixing entry ${entry.agentId}:${entry.period} - adding missing fields`
            );
            ...
            if (periodMatch) {
              ...
              metrics.elevenlabsSuccessRate =
                metrics.elevenlabsSuccessRate || 0;
              metrics.lastUpdated = metrics.lastUpdated || new Date();

              fixedCount++;
            } else {
              console.log(
                `[Data-Migration] Removing entry with invalid period format: ${entry.period}`
              );
              removedCount++;
              continue;
            }
          ...
          if (
            !entry.source ||
            !["elevenlabs", "ghl", "internal", "combined"].includes(
              entry.source
            )
          ) {
        ...
        try {
          await client.save();
          console.log(
            `[Data-Migration] Saved client ${client.clientId} - removed ${removedCount}, fixed ${fixedCount}`
          );
        } catch (saveError) {
          console.error(
            `[Data-Migration] Error saving client ${client.clientId}:`,
            saveError
          );
...
export async function validateClientMetrics(clientId) {
  try {
    ...
    for (const [index, entry] of client.metricsHistory.entries()) {
      ...
      if (
        entry.source &&
        !["elevenlabs", "ghl", "internal", "combined"].includes(entry.source)
      ) {
